...
────────────────────────
WhatsApp Cloud API (Meta official) sender + inbound processing helpers.
...
Dormant-by-design: when credentials are absent every helper raises
...
async def send_text(
    ...
    """
    Send a plain text message. Returns the Graph API JSON response
    ({messaging_product, contacts, messages}).
    """
    return await _send_message(
        {"type": "text", "text": {"body": body}}, to, client
    )
...
async def send_audio(
    ...
    so callers must host the WAV at a publicly reachable HTTPS URL.
    """
    return await _send_message(
